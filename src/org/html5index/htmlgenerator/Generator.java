...
public class Generator {
	...
	static HtmlWriter createWriter(String name) throws IOException {
		HtmlWriter writer = new HtmlWriter(new BufferedWriter(
				new OutputStreamWriter(new FileOutputStream("gen/" + name
						+ ".html"), "utf-8")));
	...
	public void writeModel() throws IOException {
		...
		for (Library lib : model.getLibraries()) {
			writer.markup("<li><a href='").text(lib.getName() + ".html")
					.markup("' target='lib'>");
	...
	public void writeIndex() throws IOException {
		HtmlWriter writer = new HtmlWriter(new OutputStreamWriter(
				new FileOutputStream("gen/index.html")));
	...
	public void writeLinkedType(HtmlWriter writer, Type type)
			throws IOException {
		if (type == null) {
			writer.text("void");
			...
			writer.text(")");
				...
				|| (type.getLibrary() != null && type.getLibrary().getName()
						.equals("hidden"))) {
			...
			writeLinkedType(writer, type.getSuperType());
			writer.text(">");
		} else if (type.getKind() == Type.Kind.PROMISE) {
			writer.text("Promise<");
			writeLinkedType(writer, type.getSuperType());
			writer.text(">");
		} else {
			writer.markup("<a href='")
					.text(type.getLibrary().getName() + " - " + type.getName()
							+ ".html").markup("' target='type'>");
			writer.text(type.getName());
			writer.markup("</a>");
	...
	public void writeLibrary(Library lib) throws IOException {
		...
		writer.markup("<h3><a href='").text(lib.getName() + " - Overview.html")
				.markup("' target='type'>");
		writer.text(lib.getName());
		writer.markup("</a></h3>\n");
		...
		for (String url[] : lib.getDocumentationProvider().getUrls()) {
			writer.markup("<li><a class='ext' href='").text(url[0])
					.markup("'>");
			writer.text(url[1]);
			writer.markup("</a></li>");
		}
	...
	int listKind(Type.Kind kind) {
		if (kind == Type.Kind.GLOBAL || kind == Type.Kind.INTERFACE
				|| kind == Type.Kind.PARTIAL || kind == Type.Kind.PARTIAL
				|| kind == Type.Kind.NO_OBJECT || kind == Type.Kind.PROPERTY
				|| kind == Type.Kind.CALLBACK_INTERFACE
				|| kind == Type.Kind.ARRAY_OBJECT) {
			return 2;
		}
	...
	public void writeTypesIndex(HtmlWriter writer, Iterable<Type> list)
			throws IOException {
		...
		/*
		 * if (globals.size() > 0) { writer.markup("<h4>Global</h4>");
		 * writer.markup("<ul class='plain'>\n"); for (Type type: globals) {
		 * writer.markup("<li>"); writeLinkedType(writer, type);
		 * writer.markup("</li>\n"); } writer.markup("</ul>\n"); }
		 */
		if (types.size() > 0) {
			writer.markup("<ul class='plain'>\n");
	...
	public void writeHeader(HtmlWriter writer, Library lib) throws IOException {
		...
		if (lib != null) {
			writer.markup("  &gt; <a href='").text(lib.getName())
					.markup(" - Overview.html'><b>");
	...
	public void writeType(Type type) throws IOException {
		HtmlWriter writer = createWriter(type.getLibrary().getName() + " - "
				+ type.getName());
		...
		if (type.getImplementedBy().size() != 0) {
			writer.markup("<p>").text(
					type.getKind() == Type.Kind.NO_OBJECT ? "Implemented by "
							: "Extended by ");
			boolean first = true;
		...
		if (type.getDocumentationSummary() != null
				&& type.getKind() != Kind.PROPERTY) {
			writer.markup("<p>").text(type.getDocumentationSummary())
					.markup("</p>");
		}

		switch (type.getKind()) {
		case PARTIAL:
			writer.markup("<p>This page describes "
					+ type.getLibrary().getName()
					+ " extensions to the origial ");
			writeLinkedType(writer, type.getSuperType());
			writer.markup(" type.</p>");
			break;
		case NO_OBJECT:
			writer.markup("<p>This type groups properties and / or operations together for documentation "
					+ "purposes and does not have an explicit JavaScript representation.</p>");
			break;
		case DICTIONARY:
			writer.markup("<p>This type represents a collection of object properties and does not have "
		...
		if (type.getKind() == Kind.PROPERTY)
			properties = type.getOwnAndCssProperties();
		else
			properties = type.getOwnAndInterfaceProperties();
		if (properties.size() != 0) {
			ArrayList<Property> constants = new ArrayList<Property>();
			ArrayList<Property> staticProperties = new ArrayList<Property>();
			ArrayList<Property> cssProperties = new ArrayList<Property>();
			...
			for (Property p : properties) {
				...
				} else if (p.hasModifier(Artifact.CSS)) {
					cssProperties.add(p);
				} else {
			...
			writeProperties(writer, cssProperties);
	...
	void writeMemberLink(HtmlWriter writer, Member m) throws IOException {
		Type owner = m.getOwner();
		writer.markup("<a href='");
		writer.text(owner.getLibrary().getName() + " - " + owner.getName()
				+ ".html#" + m.getName());
		writer.markup("'>");
	...
	void writeAboutContent(HtmlWriter writer, boolean inFrame)
			throws IOException {
		...
		writer.text(
				"Have you always missed something similar to \"JavaDoc\" for JavaScript ")
				.markup("&mdash; ");
		...
		for (Library lib : model.getLibraries()) {
			...
			writer.markup("<a href='").text(lib.getName() + " - Overview.html")
					.markup("'>");
			writer.text(lib.getName());
			writer.markup("</a>");
		}
	...
	public void writeOperations(HtmlWriter writer,
			Collection<Operation> operations) throws IOException {
		for (Operation operation : operations) {
			writer.markup("<tr id='").text(operation.getName())
					.markup("'><td>");
			...
			if (docUrl != null) {
				writer.markup("<a class='ext' href='").text(docUrl)
						.markup("'>");
	...
	public void writeProperties(HtmlWriter writer, Iterable<Property> properties)
			throws IOException {
		for (Property property : properties) {
			...
			if (property.hasModifier(Artifact.CSS))
	...
	public void writeGlobalIndex() throws IOException {
		...
		for (Artifact a : index) {
			...
			if (fc != current) {
				...
				writer.markup("<h2 id='").text(String.valueOf(fc)).markup("'>")
						.text(String.valueOf(fc)).markup("</h2>");
				writer.markup("<ul class='plain'>");
				current = fc;
			}
			writer.markup("<li>");
			if (a instanceof Type) {
				...
				writer.markup("<a href='").text(
						a.getName() + " - Overview.html");
